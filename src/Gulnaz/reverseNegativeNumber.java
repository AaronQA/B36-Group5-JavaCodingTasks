...
public class reverseNegativeNumber {
    ...
    public static int reverseNegativeNumber(int num){
        ...
        //2. It initializes a variable `reversedNegativeNumber` to 0,
        // which will store the reversed digits.
        int reverseNegativeNumber=0;

        //3. The method enters a while loop that continues until `num` becomes 0.
        while(num!=0){

            //4. Inside the loop, it calculates the last digit of `num`
            // by taking the remainder of `num` divided by 10 (`num % 10`).
            int digit= num%10;

            //5. It then updates `reversedNegativeNumber` by multiplying it
            // by 10 and adding the last digit (`reversedNegativeNumber * 10 + digit`).
            reverseNegativeNumber=reverseNegativeNumber*10 + digit;

            //6. Finally, it updates `num` by dividing it
            // by 10 (`num /= 10`) to remove the last digit from `num`.
            num/=10;
        }

        //7. Once the loop finishes, the method returns `reversedNegativeNumber`,
        // which contains the reversed digits of the negative number.
